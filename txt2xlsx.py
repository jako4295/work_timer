...
def check_start_stop_shift(lines, filename):
    ...
    for i, line in enumerate(lines):
        if i%2 == 0:
            if line[:5] != 'start':
                raise Exception(f"In file {filename}, an even index started with something other than 'start' which is unexpected")
        else:
            if line[:4] != 'stop':
                raise Exception(f"In file {filename}, an odd index started with something other than 'stop' which is unexpected")
    pass
...
def round_time(lines, time='15min'):
    ...
    for i, line in enumerate(lines):
        if i%2 == 0:
            rounded_lines.append('start '+str(pd.Timestamp(line[6:]).round(time))[:-3])
        else:
...
for file_temp in files:
    ...
    if len(round_lines) <= 2:
        ...
        for i in range(2, len(round_lines), 2):
            pause += pd.Timestamp(round_lines[i][-5:]) - pd.Timestamp(round_lines[i-1][-5:])

    total_time = (pd.Timestamp(stop_time) - pd.Timestamp(start_time)) - pd.Timedelta(pause)
...
with pd.ExcelWriter(folder+'hours.xlsx') as writer:
    df_excel.style.applymap(lambda _: 'text-align: center').to_excel(writer, sheet_name='timer', index=False)

    # Auto-adjust columns' width
    for column in df_excel:
        column_width = max(df_excel[column].astype(str).map(len).max(), len(column))*1.1
